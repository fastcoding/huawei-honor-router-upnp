...
function uPnPclient:discoverIGD()
    ...
    if #services == 0 then
	return nil, "ERROR: Unable to find service nodes in: " .. (dev_http_response.body or '')
    end

    self:debug(9, "Services: ", services)
    self:debug(1, 'services:', #services)
	...
	return nil, "ERROR: Unable to find serviceType WANIPConnection or WANPPPConnection in: " .. (dev_http_response and dev_http_response.body or '')
    end
...
function uPnPclient:SendSoapRequest(soap_body, action)
	...
	if dev_http_response.err:match('^<s:Envelope') then
	    ...
	    for k,v in pairs({ "s:Envelope",1, "s:Body", "s:Fault", 'detail', 'UPnPError' }) do
		if err[v] == nil then
		    break
		end
...
function uPnPclient:GetExternalIPAddress()
    ...
    print('external ret:',inspect(ret))
    return ret
end
